...
interface IIdentityBulkRegistrar {
    function registerFor(address to, string calldata agentURI) external returns (uint256 agentId);
}

/// @notice Bulk-register Old School agents through the live registrar/admin path.
/// @dev Requires the broadcaster to already hold REGISTRAR_ROLE on the identity registry.
contract BulkRegisterAgents is Script {
    ...
    function run() external {
        ...
        string memory ownerString = json.readString(".ownerAddress");
        ...
        address owner = vm.parseAddress(ownerString);

        vm.startBroadcast();
        for (uint256 i; i < agents.length; i++) {
            string memory agentURI = _buildAgentURI(agents[i], ownerString, _pricingForAgent(agents[i], defaultPricing));
            uint256 agentId = IIdentityBulkRegistrar(IDENTITY).registerFor(owner, agentURI);
