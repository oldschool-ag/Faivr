...
async function stripeRequest(path:string, init:RequestInit={}) {
  ...
  const response=await fetch(`https://api.stripe.com/v1/${path}`,{...init,headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/x-www-form-urlencoded",...(init.headers||{})}});
  const value=await response.json() as Record<string,unknown>;
  if(!response.ok) throw new Error(typeof value.error==="object"&&value.error&&"message" in value.error?String((value.error as {message:unknown}).message):"stripe_provider_error");
  return value;
}
